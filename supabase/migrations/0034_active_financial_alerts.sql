create table if not exists public.financial_alerts (
  ...
  constraint financial_alerts_kind_check check (
    kind in ('negative_balance', 'due_bill', 'unusual_spending')
  ),
  constraint financial_alerts_severity_check check (severity in ('info', 'warning', 'danger')),
  constraint financial_alerts_status_check check (status in ('active', 'dismissed', 'resolved')),
  constraint financial_alerts_workspace_fingerprint_unique unique (workspace_id, fingerprint)
);

create index if not exists financial_alerts_workspace_status_idx
on public.financial_alerts(workspace_id, status, severity, last_seen_at desc);

create index if not exists financial_alerts_kind_idx
on public.financial_alerts(kind, status);

alter table public.financial_alerts enable row level security;

grant select, insert, update on table public.financial_alerts to authenticated;

drop policy if exists "Workspace users can read active financial alerts" on public.financial_alerts;
create policy "Workspace users can read active financial alerts"
...
using (
  ...
  or exists (
    ...
    where workspaces.id = financial_alerts.workspace_id
    and workspaces.owner_id = auth.uid()
  )
  or public.is_active_workspace_member(workspace_id)
);

drop policy if exists "Workspace users can create financial alerts" on public.financial_alerts;
create policy "Workspace users can create financial alerts"
on public.financial_alerts
for insert
with check (
  public.is_saas_admin()
  ...
  or public.can_manage_workspace_finance(workspace_id)
);
...
drop policy if exists "Workspace users can update financial alerts" on public.financial_alerts;
create policy "Workspace users can update financial alerts"
on public.financial_alerts
for update
using (
  public.is_saas_admin()
